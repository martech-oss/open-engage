...
for (const file of files) {
  ...
  if (
    ...
    !isTest &&
    (imports.includes("@orpc/client/fetch") ||
      /\b(?:new\s+RPCLink|createORPCClient|createTanstackQueryUtils)\s*\(/.test(source))
  ) {
    violations.push(
...
function extractAstFacts(sourceFile) {
  ...
  function visit(node) {
    ...
    if (
      ...
      staticPropertyName(node.name) === "ssr" &&
      unwrapTransparentExpression(node.initializer).kind === SyntaxKind.FalseKeyword
    ) {
      hasRouteSsrDisabled = true;
    }
    if (isDirectRolePermissionExpression(node)) hasClientRolePermissionLogic = true;
...
function isDirectRolePermissionExpression(node) {
  if (isNewExpression(node) && isRolePermissionCollection(node)) return true;
  if (
    isBinaryExpression(node) &&
    [
    ...
    return (
      (isRoleReference(node.left) && isWorkspaceRoleLiteral(node.right)) ||
      (isRoleReference(node.right) && isWorkspaceRoleLiteral(node.left))
    );
  }
  if (
    ...
    if (
      ...
      node.arguments.some(isRoleReference) &&
      isRolePermissionCollection(callee.expression)
    ) {
      return true;
    }
  }
...
function isRolePermissionCollection(node) {
  const target = unwrapTransparentExpression(node);
  ...
  if (
    ...
    return target.arguments?.some(isRolePermissionCollection) ?? false;
  }
  return isIdentifier(target) && /(?:role|permission)/i.test(target.text);
}
