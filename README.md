...
- 行動加点のルール別上限と日数による減衰（[設定・計算方法](docs/scoring.md)）
- 初回訪問者にも表示できるCTAと再表示制御（[サイトメッセージ](docs/site-messages.md)）
...
- 流入元: 保存中の初回流入元別のPV・フォーム・MQL・商談・受注金額（[集計範囲と保持期間](docs/acquisition-reporting.md)）
...
正の加点ルールには、連絡先ごとのルール加点上限と、0点になるまでの減衰日数を設定できます。減衰は元のイベント日時から日単位で計算し、Cronで総点・カテゴリ点・履歴に反映します。既存の手動調整は保持されます。設定変更は以後の加点に適用されます。詳細は[スコアの減衰・上限](docs/scoring.md)を参照してください。
...
- Projectのアトリビューションは紐付けたリソースへの反応を接点とします。別の「流入元」レポートは保存中のページ・フォーム履歴を使うため、履歴の削除・アーカイブ後に過去の集計先が変わることがあります。
