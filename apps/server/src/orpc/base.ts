...
import { WorkspaceAccessError, type WorkspaceAccess } from "../auth/access";
...
function createAccessMiddleware(
  resolve: (context: OrpcInitialContext) => Promise<WorkspaceAccess>,
...
const requireWorkspace = createAccessMiddleware(
  (context) => context.access.workspace(),
  (code) => {
    switch (code) {
...
const requireSessionWorkspace = createAccessMiddleware(
  (context) => context.access.sessionWorkspace(),
  (code) => {
    switch (code) {
      case "origin_mismatch":
        return "ORIGIN_MISMATCH";
...
const requireSession = os.middleware(async ({ context, next, errors }) => {
  try {
    const access = await context.access.session();
    return next({ context: access });
  } catch (error) {
    if (!(error instanceof WorkspaceAccessError)) throw error;
