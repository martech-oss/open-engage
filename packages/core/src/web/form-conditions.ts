...
export function resolveFormFields(
  ...
  function matches(condition: FormCondition | undefined): boolean {
    if (!condition) return true;
    const raw = values[condition.field];
    const value =
      isVisible(condition.field) &&
      (typeof raw === "string" || typeof raw === "number" || typeof raw === "boolean")
        ? String(raw).trim()
        : "";
    if (condition.operator === "empty") return value === "";
    if (condition.operator === "not_empty") return value !== "";
    return condition.operator === "equals" ? value === condition.value : value !== condition.value;
  }
  function isVisible(key: string): boolean {
    ...
    checking.add(key);
    const result = matches(field.visibleWhen);
    checking.delete(key);
  ...
  return fields
    .filter((field) => {
      if (!isVisible(field.key)) return false;
    ...
    .map((field) => ({
      ...field,
      required: field.required || Boolean(field.requiredWhen && matches(field.requiredWhen)),
    }));
}
