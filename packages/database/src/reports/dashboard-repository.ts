...
export class DashboardReportsRepository extends ReportsBatchRepository {
  ...
  public async dashboardSummary(
    workspaceId: string,
    input: { fromTimestamp: string; toExclusiveTimestamp: string; asOf: string },
    ...
    const [contactRows, automationRows, briefRows, deliveryRows, eventRows] = await this.runBatch(
      ...
      sql`
        ...
        WHERE ${projectBriefs.workspaceId} = ${workspaceId}
          ...
          AND ${projectBriefs.reviewAt} < ${input.asOf}
      ...
      sql`
        ...
        FROM ${deliveries}
        WHERE ${deliveries.workspaceId} = ${workspaceId}
          AND ${deliveries.createdAt} >= ${input.fromTimestamp}
          AND ${deliveries.createdAt} < ${input.toExclusiveTimestamp}
      `,
      sql`
